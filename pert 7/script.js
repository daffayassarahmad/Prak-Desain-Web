...
function tampilkanDaftarBuku() {
  ...
  daftarBuku.forEach((buku, index) => {
    ...
    divBuku.innerHTML = `
              ...
              <button class="bg-blue-500 text-white px-3 py-1 rounded-lg hover:bg-blue-600 transition duration-300" onclick="tambahKeFavorit(${index})">Tambah ke Favorit</button>
          `;
...
function tampilkanBukuFavorit() {
  ...
  bukuFavorit.forEach((buku, index) => {
    ...
    divBuku.innerHTML = `
              <p>${buku.tampilkanInfo()}</p>
              <button class="bg-red-500 text-white px-3 py-1 rounded-lg hover:bg-red-600 transition duration-300" onclick="hapusDariFavorit(${index})">Hapus</button>
          `;
